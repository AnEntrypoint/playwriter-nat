...
class CLI {
      ...
      .command('serve', 'Run as relay server (default)', (yargs) => {
        return yargs
          ...
          .option('seed', {
            alias: 's',
            type: 'string',
            description: 'Seed for deterministic DHT key generation (enables key persistence across restarts)'
          })
          .option('host', {
            type: 'string',
            default: '0.0.0.0',
  ...
  static async handleServeCommand(argv) {
    ...
    try {
      const { publicKey } = await relay.startServer(token, 'localhost', seed);
