...
class PlaywriterRelay {
  ...
  forwardClientToServe(clientId, socket) {
    ...
    socket.on('data', (data) => {
      ...
      try {
        const unpacker = packr.unpack(buffer);
        if (unpacker !== undefined) {
          const msg = unpacker;
          if (msg && typeof msg === 'object' && msg.id) {
            clientInfo.messageIds.add(msg.id);
            this.messageIdMap.set(msg.id, clientId);
          }

          const jsonData = Buffer.from(JSON.stringify(msg) + '\n');
          this.writeToServe(jsonData).catch((err) => {
            if (!clientInfo.closed) {
        ...
        console.error(`[${clientId}] Deserialize error:`, e.message);
        buffer = Buffer.alloc(0);
      }
    });
      ...
      try {
        ...
        if (msg && typeof msg === 'object' && msg.id) {
          ...
          if (targetClientId === clientId) {
            ...
            if (msg.method === 'createPage' && msg.result) {
              const pageId = msg.result.pageId;
              if (pageId) {
  ...
  async setupStdioForwarding(socket) {
    return new Promise((resolve, reject) => {
      ...
      // Handle stdin → socket (encode to msgpackr)
      process.stdin.on('data', (data) => {
        ...
        while (stdinBuffer.length > 0) {
          ...
          const line = stdinBuffer.slice(0, newlineIdx).toString();
      ...
      // Handle socket → stdout (decode from msgpackr)
      socket.on('data', (data) => {
        socketBuffer = Buffer.concat([socketBuffer, data]);

        try {
          const msg = packr.unpack(socketBuffer);
          if (msg !== undefined) {
            process.stdout.write(JSON.stringify(msg) + '\n');
            socketBuffer = Buffer.alloc(0);
          }
        } catch (e) {
          console.error('[client] msgpackr unpack error:', e.message);
        }
      });
