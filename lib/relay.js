...
class PlaywriterRelay {
  ...
  async startServer(token, playwrightHost = 'localhost', seed = null) {
    ...
    try {
      ...
      console.log('[relay] Clearing port 19988...');
      try {
        execSync('pkill -9 -f "playwriter serve" 2>/dev/null || true');
  ...
  forwardClientToServe(clientId, socket) {
    ...
    const mcpProcess = spawn('npx', ['-y', 'playwriter@latest', 'mcp', '--host', 'localhost', '--port', '19988', '--token', this._serveToken], {
    ...
    console.log(`[${clientId}] MCP initializing (waiting 1s before stdio forwarding)`);
  }
