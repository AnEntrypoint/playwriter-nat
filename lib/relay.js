...
class PlaywriterRelay {
  ...
  forwardClientToServe(clientId, socket) {
    ...
    const clientPort = 20000 + this.clients.size;
    const env = { ...process.env, PLAYWRITER_PORT: clientPort.toString(), PORT: clientPort.toString() };

    const mcpProcess = spawn('npx', ['-y', 'playwriter@latest', 'mcp', '--host', 'localhost:' + this._servePort, '--token', this._serveToken], {
