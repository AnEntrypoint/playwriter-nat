...
class PlaywriterRelay {
  ...
  async _attemptRecovery() {
    ...
    console.log(`[relay] Waiting for playwriter serve (attempt ${this._recoveryAttempts}/${this._maxRecoveryAttempts})...`);

    await new Promise(resolve => setTimeout(resolve, backoffMs));

    if (!this._closing && !this._interrupting) {
      console.log('[relay] Ensure playwriter serve is running on localhost:' + (this._servePort || 19988));
    }
  }
  ...
  async shutdown() {
    ...
    if (this.serveProcess && !this.serveProcess.killed) {
      console.log('[relay] Terminating playwriter serve process...');
      this.serveProcess.kill('SIGTERM');

      await new Promise(resolve => {
        const timeout = setTimeout(() => {
          if (this.serveProcess && !this.serveProcess.killed) {
            console.log('[relay] Force killing playwriter serve process...');
