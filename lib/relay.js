...
class PlaywriterRelay {
  ...
  async startServer(token, playwrightHost = 'localhost', seed = null) {
    ...
    try {
      ...
      // Kill any lingering playwriter serve process on port 19988
      try {
        const { execSync } = require('child_process');
        try {
          execSync('pkill -9 -f "playwriter serve" 2>/dev/null || true', { stdio: 'ignore' });
        } catch (e) {
          // Ignore
        }
        // Wait for TCP sockets to fully close (necessary for SO_LINGER and TIME_WAIT)
